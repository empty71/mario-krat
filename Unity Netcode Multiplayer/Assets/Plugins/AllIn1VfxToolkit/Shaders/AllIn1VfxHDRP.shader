Shader "AllIn1Vfx/AllIn1VfxHDRP"
{
    Properties
    {
    ...
    SubShader
    {
        Tags
        {
            "RenderPipeline"="HDRenderPipeline" "Queue" = "Transparent" "CanUseSpriteAtlas" = "True" "IgnoreProjector" = "True" "RenderType" = "Transparent" "PreviewType" = "Plane"
        }
        ...
        Pass
        {
            ...
            v2f vert(appdata v)
            {
                ...
                #if RIM_ON || BACKFACETINT_ON || LIGHTANDSHADOW_ON
                o.viewDir = GetWorldSpaceNormalizeViewDir(TransformObjectToWorld(v.vertex));
            ...
            half4 frag(v2f i) : SV_Target
            {
				...
				half sceneDepthDiff = LinearEyeDepth(SampleCameraDepth(depthScreenPosNorm.xy),_ZBufferParams) - ComputeGrabScreenPos(depthScreenPos).a;
                #endif
            	...
            	half3 distortCol = SampleCameraColor(i.screenCoord.xy, 0);
            	#if DISTORTONLYBACK_ON
				...
				half frontMask = LinearEyeDepth(SampleCameraDepth(backDistortScreenPosNorm.xy),_ZBufferParams) - ComputeGrabScreenPos(backDistortScreenPos).a;
            	frontMask = 1 - saturate(frontMask);
            	col.rgb = lerp(lerp(col.rgb, saturate(distortCol.rgb), _DistortionBlend), lerp(col.rgb, SampleCameraColor((originalScreenCoord / originalScreenCoord.w).xy, 0).rgb, _DistortionBlend), frontMask);
            	#else
